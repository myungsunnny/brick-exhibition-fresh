...
export default async function handler(request, response) {
  if (request.method === 'GET') {
    try {
      ...
      const artworks = rawArtworks.filter(a => a).map(a => JSON.parse(a));
  ...
  if (request.method === 'POST') {
    try {
      ...
      if (!artwork || !artwork.id) return response.status(400).json({ success: false, error: '작품 정보가 올바르지 않습니다.' });
  ...
  if (request.method === 'DELETE') {
    try {
        ...
        if (!artworkId) return response.status(400).json({ success: false, error: '삭제할 작품 ID가 없습니다.' });
